...
const CHILD_PROCESS_MAX_DURATION = 300000;
...
module.exports = function(cmd, input, iii, callback) {
  ...
  setTimeout(() => {
    ...
    if(cmdProcess){
      ...
      // callback(null, {stdout, stderr, exitCode});
      stderr += '"Process killed/exited, as program took long time to complete"';
      cmdProcess.kill('SIGTERM');
      cmdProcess = null;
    }
  }, (input.TIMEOUT_DURATION_MS || CHILD_PROCESS_MAX_DURATION));
};
